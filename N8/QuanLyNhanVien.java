...
public class QuanLyNhanVien {
    protected static ArrayList<NhanVien> danhSachNhanVien;
        ...
        try {
            ...
            while (jain.hasNextLine()) {
                ...
                if (get[0].charAt(1) == 'L' && get[0].charAt(0) == 'Q') {
                    temp.add(new QuanLy(get[0], get[1], Integer.parseInt(get[2]), get[3], Double.parseDouble(get[4]),
                            Integer.parseInt(get[5])));
                }
                if (get[0].charAt(1) == 'C' && get[0].charAt(0) == 'P') {

                    temp.add(
                            new PhiCong(get[0], get[1], Integer.parseInt(get[2]), get[3], Integer.parseInt(get[4]),
                                    Integer.parseInt(get[5])));
                }
    ...
    public ArrayList<NhanVien> luongNNhanVienCaoNhat(int n, String loaiNhanVien) {
        ...
        for (int j = 0; j < n; j++) {
            for (String i : time.keySet()) {
                if (i.equals(n_List.get(0).getMaNhanVien()))
                    time_ = Integer.parseInt(time.get(i));
            }
            Double max = n_List.get(0).tongLuong(time_);
            for (int i = 0; i < n_List.size(); i++) {
                for (String runInTime : time.keySet()) {
                    if (runInTime.equals(n_List.get(i).getMaNhanVien()))
                        time_ = Integer.parseInt(time.get(runInTime));
                }
